        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="(guru, index) in gurus" :key="index">
              ...
              <td>{{ guru.address }}</td>
              <td>
                <button class="btn btn-primary btn-sm" v-on:click="editGuru(guru)">
                  <span class="fa fa-edit"></span>
                </button>
                <button class="btn btn-danger btn-sm" v-on:click="deleteGuru(guru)">
  ...
  methods: {
    ...
    updateGuru: async function() {
      try {
        ...
        const response = await guruService.updateGuru(
          this.editGuruData.id,
          formData
        );
        this.gurus.map(guru => {
