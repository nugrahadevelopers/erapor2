        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="(kelas, index) in kelass" :key="index">
              ...
              <td>{{ kelas.kuota }}/32</td>
              <!-- <td v-for="(siswa, index) in kelas.siswa" :key="index">{{ siswa.nama }}</td> -->
              <td>
                <router-link
                  :to="{ name: 'tambahsiswa', params: { kelasid:kelas.id, kelasnama:kelas.nama_kelas, kelastingkat:kelas.tingkat }}"
